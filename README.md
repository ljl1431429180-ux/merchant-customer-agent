...
1. 在支持可信身份注入的测试环境部署商家客服中心后台，并绑定 D1 数据库；
...
> **身份边界**：当前公开版仅信任 ChatGPT Sites 注入的登录身份，并允许 `localhost` 本地开发。部署到其他服务器时，受保护接口会默认拒绝访问；必须先接入自己的 OAuth、登录会话或其他服务端可验证身份机制，不能只在浏览器请求中自行填写身份头。
...
本项目使用 Cloudflare D1 作为数据层。在受信任的测试环境部署时，需要创建并绑定名为 `DB` 的 D1 数据库，并按 `drizzle/` 中的迁移文件初始化表结构。若迁移到其他服务器，还必须先实现正式登录和服务端会话验证。
...
- 受保护接口会校验可信部署主机、同源来源和平台注入的身份；当前仅支持 ChatGPT Sites 测试环境与 localhost 开发，其他生产部署必须替换为正式认证方案；
